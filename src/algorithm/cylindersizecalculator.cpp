...
void CylinderSizeCalculator::calibrate(const std::vector<Line> &inLines, double distance, double radius, double height)
{
    lines = inLines;
    ...
    double visibleEdge = distance - (radius + radius*radius/(distance - radius));
...
void CylinderSizeCalculator::calculateSize()
{
    ...
    auto localFactor = factor*(distance - (radius + radius*radius/(distance-radius)));
